...
def upward_continue(
    bmap: NDArray, uc_distance: float, pixel_size: float = 1.175e-06, oversample: int = 2
) -> NDArray:
    ...
    # this is a grid from [0... pixel / 2,  -pixel / 2 ... 0]
    ...
    x_filter = np.exp(-uc_distance * k)

    # Compute FFT of the field map
    fft_map = np.fft.fft2(bmap, s=(new_y, new_x))

    # Calculate x component
    B_out = np.fft.ifft2(fft_map * x_filter)
    LOG.debug("Upward continued map by %s m", uc_distance)
    # Crop matrices to get rid of zero padding
    return B_out[:ypix, :xpix].real


def main():
    synth_map = np.random.normal(100, 100)
    upward_continue(synth_map, 1e6)
